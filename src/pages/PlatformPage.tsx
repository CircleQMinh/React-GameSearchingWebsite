...
function PlatformPage() {
  ...
  var id = params.id;
  let name: string = "";
  name = platforms.filter((q) => q.id.toString() == id)[0]?.name;
  ...
  useEffect(() => {
    var query = generateQueryGamesByPlatform(number - 1, Number.parseInt(id!));
    console.log(query);
  ...
  useEffect(() => {
    if (id == undefined) {
      ...
      if (!platforms.some((g) => g.id.toString() == id)) {
        navigate("/error");
      }
    }
  });
