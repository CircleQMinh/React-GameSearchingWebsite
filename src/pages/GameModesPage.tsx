...
import { GameModes } from "../constants/game_mode";
import { Genres } from "../constants/genre";
import { useAppDispatch, useAppSelector } from "../hook/redux";
import { getSearchResultAsync } from "../redux/Game/reducer";
import { generateQueryGamesByGameMode } from "../utils/generateQuery";
...
function GameModesPage() {
  ...
  var id = params.id;
  let name: string = "";
   name = gamemodes.filter((q) => q.id.toString() == id)[0]?.name;
  ...
  useEffect(() => {
    var query = generateQueryGamesByGameMode(number - 1, Number.parseInt(id!));
  ...
  useEffect(() => {
    if (id == undefined) {
      ...
      if (!gamemodes.some((g) => g.id.toString() == id)) {
        navigate("/error");
      }
    }
  });
